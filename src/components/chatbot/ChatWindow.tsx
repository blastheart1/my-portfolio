...
export const ChatWindow: React.FC<ChatWindowProps> = ({
  ...
  // Handle horizontal scrolling with mouse wheel on ENTIRE badges section (DESKTOP ONLY)
  useEffect(() => {
    const badgesSection = badgesSectionRef.current;
    const badgesContainer = badgesScrollRef.current;
    if (!badgesSection || !badgesContainer || isMobile) return;

    const handleWheel = (e: WheelEvent) => {
      // CRITICAL: Prevent ALL scroll behavior
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
      
      // Horizontal scrolling with increased sensitivity
      badgesContainer.scrollLeft += e.deltaY * 2.3;
      
      return false;
    };

    // Attach to the ENTIRE section (not just badges container)
    badgesSection.addEventListener('wheel', handleWheel, { passive: false, capture: true });
    
    return () => {
      badgesSection.removeEventListener('wheel', handleWheel, { capture: true });
    };
  }, [isMobile, isOpen]);

  // Prevent messages container scroll from leaking to portfolio (DESKTOP ONLY)
  useEffect(() => {
    ...
    const handleWheel = (e: WheelEvent) => {
      ...
      // ALWAYS stop propagation to prevent portfolio scroll
      e.stopPropagation();
      e.stopImmediatePropagation();
      
      // If at boundaries and trying to scroll past, also prevent default
      if ((isAtTop && e.deltaY < 0) || (isAtBottom && e.deltaY > 0)) {
        e.preventDefault();
        return false;
      }
      
      // If not scrollable at all, prevent default too
      if (!isScrollable) {
        e.preventDefault();
        return false;
      }
    ...
    return () => {
      messagesContainer.removeEventListener('wheel', handleWheel, { capture: true });
    };
  }, [isMobile, isOpen]);
  ...
  // Check if we're in an iframe (embed context) - SSR-safe
  const isInIframe = typeof window !== 'undefined' ? window.parent !== window : false;
